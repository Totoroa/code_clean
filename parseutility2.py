...
def removeComment(string):
    ...
    s = ''.join([c.group('noncomment') for c in c_regex.finditer(string) if c.group('noncomment')])
    s = s.replace('//-', '')
    return s
...
def abstract_slice(slice_content, var_list):
    '''
    slice_content: String
    var_list: List
    
    return slice_content: String
    '''
    slice_content = slice_content.replace('//-', '')
    for lvar in var_list:
            if len(lvar) == 0:
                continue
            try:
                lvarPattern = re.compile("(^|\W)" + lvar + "(\W)")
                slice_content = lvarPattern.sub("\g<1>LVAR\g<2>", slice_content)
            except:
                pass
...
def abstract(instance, level):
    ...
    originalFunctionBody = originalFunctionBody.replace('//-', '')
    # print "==================="
